...
> **Note:** The templates below are illustrative pseudocode — they elide the full `Element` trait parameters (`global_id`, `inspector_id`) and simplify child-element calls. Verify exact signatures against the gpui source; e.g. `AnyElement::request_layout(window, cx)`, `AnyElement::prepaint_at(origin, window, cx)`, and `AnyElement::paint(window, cx)` (no bounds argument). Closures given to `window.on_mouse_event` are `'static` — they cannot borrow `&mut self`; mutate state through a cloned `Entity` (`entity.update(cx, |state, cx| { ...; cx.notify(); })`).
        ...
        -> (LayoutId, Vec<LayoutId>)
    {
        ...
        let layout_id = window.request_layout(
            Style {
                ...
                gap: size(self.gap, self.gap).into(), // Style.gap is Size<DefiniteLength>
                ..default()
            },
...
pub struct InteractiveElement {
    id: ElementId,
    content: AnyElement,
    on_click: Option<Rc<dyn Fn(&MouseUpEvent, &mut Window, &mut App)>>,
             ...
             prepaint: &mut (Hitbox, bool), window: &mut Window, cx: &mut App)
    {
        ...
        if *is_hovered {
            if let Some(hover_color) = self.hover_color {
                window.paint_quad(fill(bounds, hover_color).corner_radii(px(4.)));
            }
        }
...
impl Element for ScrollableElement {
             ...
             paint_state: &mut ScrollPaintState, window: &mut Window, cx: &mut App)
    {
        ...
        // NOTE: illustrative — this closure is 'static and cannot really borrow
        // `&mut self`; in real code keep the offset in an Entity (or ScrollHandle)
        // cloned into the closure and call entity.update(cx, ...).
        window.on_mouse_event({
