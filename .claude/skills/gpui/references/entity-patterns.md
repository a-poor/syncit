...
impl DataLoader {
    ...
    fn load_data(&mut self, cx: &mut Context<Self>) {
        ...
        // From Context<Self>, cx.spawn hands the closure a WeakEntity<Self>
        // and an &mut AsyncApp (app/context.rs:237) — no manual downgrade needed.
        // (From a plain &mut App, the shape is cx.spawn(async move |cx| ...)
        // with only the &mut AsyncApp.)
        cx.spawn(async move |this, cx| {
            let result = fetch_data().await;

            // WeakEntity::update returns Result — Err if the entity was dropped
...
impl ImageProcessor {
    fn process_images(&mut self, paths: Vec<PathBuf>, cx: &mut Context<Self>) {
        ...
        // Run the heavy work on the background executor. Task has no `.then`
        // combinator (executor.rs) — await the background task inside a
        // foreground spawn instead.
        ...
        cx.spawn(async move |this, cx| {
            ...
            // Update entity on the foreground thread
            let _ = this.update(cx, |state, cx| {
...
Use a coordinator entity to fan events out to components.

Note: `Entity<T>` / `WeakEntity<T>` require a sized `'static` type (`Entity` stores `PhantomData<fn(T) -> T>`, app/entity_map.rs:414), so `WeakEntity<dyn Trait>` does not compile. Instead of a trait-object listener list, make the coordinator an `EventEmitter` and let each interested component subscribe.
...
impl EventEmitter<AppEvent> for EventCoordinator {}
...
// Each listener subscribes to the shared Entity<EventCoordinator>:
impl SomeListener {
    fn new(coordinator: Entity<EventCoordinator>, cx: &mut Context<Self>) -> Self {
        cx.subscribe(&coordinator, |this, _coordinator, event: &AppEvent, cx| {
            match event {
                AppEvent::SomethingHappened => { /* react */ }
            }
        })
        .detach();
        Self { /* ... */ }
